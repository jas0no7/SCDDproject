...
def runpanoramaOverview():
    ...
    and cs.operation_status in ('停运','投运')
    """
    DF_SCDD = SQL(sql)
    ...
    # 枪数量合并
    DF_SCDD['charge_point_count'] = DF_SCDD['dc_charge_point_count'].fillna(0) + DF_SCDD[
    ...
    print('当前四川电动停运、投运状态中的充电站数量：', DF_SCDD.shape[0])
    ...
    # 计算截至到上年同月的新增枪数量
    d2_1 = DF_SCDD[(DF_SCDD['year_month'] <= last_year_month_str) & (DF_SCDD['year'] == last_year)][
        'charge_point_count'].sum()
    print('上年新增枪数量：', d2_1)
    # 计算同比增长
    ...
    d5 = DF_SCDD[(DF_SCDD['year'] == year) & (DF_SCDD['year_month'] <= M)].groupby('station_category').agg(
        {'charge_point_count': 'sum'}).reset_index().sort_values(by='charge_point_count', ascending=False).iloc[0][
        'station_category']
    data1.append(d5)
    data1
    ...
    d6 = DF_SCDD[(DF_SCDD['year'] == year) & (DF_SCDD['year_month'] <= M)].groupby('city').agg(
        {'charge_point_count': 'sum'}).reset_index().sort_values(by='charge_point_count', ascending=False).iloc[0][
        'city']
    data1.append(d6)
    data1
    ...
    d7 = DF_SCDD[DF_SCDD['year_month'] <= M].groupby('year').agg(
        {'charge_point_count': 'sum'}).reset_index().sort_values(by='year', ascending=False).head(5)
    ...
    sql = """
    select b.*,a.city,a.station_category,a.dc_charge_point_count,a.ac_charge_point_count,
    a.operation_status as station_operation_status from
    (SELECT 
    cs.*
    FROM
    charging_station cs
    where 
    cs.merchant_nature = '电动公司'
    and cs.operation_status in ('停运','投运','退运')
    ) a
    left join 
    ...
    DF_cba_org_data['other_revenue_battery_swap_services'] = DF_cba_org_data[
        'other_revenue_battery_swap_services'].astype(str).astype(float)
    ...
    M = str(M)
    d4 = DF_cba_org_data[(DF_cba_org_data['cba_month'] <= M) & (DF_cba_org_data['year'] == str(year))][
        ['rec_data']].sum().sum()
    ...
    d5_1 = DF_cba_org_data[
        (DF_cba_org_data['cba_month'] <= last_year_month_str) & (DF_cba_org_data['year'] == str(last_year))][
        ['rec_data']].sum().sum()
    ...
    d6 = DF_cba_org_data[DF_cba_org_data['cba_month'] == M].groupby('station_category').agg(
        {'rec_data': 'sum'}).reset_index().sort_values(by='rec_data', ascending=False).iloc[0]['station_category']
    data2.append(d6)
    data2
    ...
    d7 = DF_cba_org_data[DF_cba_org_data['cba_month'] == M].groupby('city').agg(
        {'rec_data': 'sum'}).reset_index().sort_values(by='rec_data', ascending=False).iloc[0]['city']
    data2.append(d7)
    data2
    ...
    DF1 = pd.DataFrame(
        columns=['month_revenue', 'yoy_growth_1', 'mom_growth', 'year_revenue', 'yoy_growth_2', 'top1_type',
                 'top1_city'], data=[data2])
    ...
    sql = """
    select * from
    (select cs.station_no,cs.station_category from charging_station cs
    where cs.merchant_nature = '电动公司'
    and cs.operation_status in ('停运','投运')) c
    inner join
    ...
    d4 = DF_operation_duration[(DF_operation_duration['month'] <= M) & (DF_operation_duration['year'] == str(year))][
        '可用率'].mean()
    ...
    d5_1 = DF_operation_duration[
        (DF_operation_duration['month'] <= last_year_month_str) & (DF_operation_duration['year'] == str(last_year))][
        '可用率'].mean()
    d5 = f"{(d4 / d5_1 - 1) * 100:.2f}"
    data4.append(d5)
    data4
    ...
    d6 = DF_operation_duration[DF_operation_duration['month'] == M].groupby('station_category').agg(
        {'可用率': 'mean'}).reset_index().sort_values(by='可用率', ascending=False).iloc[0]['station_category']
    data4.append(d6)
    data4
    ...
    d7 = DF_operation_duration[DF_operation_duration['month'] == M].groupby('city').agg(
        {'可用率': 'mean'}).reset_index().sort_values(by='可用率', ascending=False).iloc[0]['city']
    data4.append(d7)
    data4
    ...
    DF_cba_org_data['charge_point_count'] = DF_cba_org_data['dc_charge_point_count'].fillna(0) + DF_cba_org_data[
        'ac_charge_point_count'].fillna(0)
    ...
    # 当月单枪充电量，日均的计算在后面
    DF_cba_org_data['gun_charging_volume'] = DF_cba_org_data['plat_data_charging_volume'] / DF_cba_org_data[
        'charge_point_count']
    ...
    d4_1 = DF_cba_org_data[
        (DF_cba_org_data['cba_month'].str[:4] == str(year)) & (DF_cba_org_data['cba_month'] <= M)].groupby(
        'cba_month').agg({"gun_charging_volume": 'mean'}).reset_index().copy()
    ...
    d5_1 = DF_cba_org_data[(DF_cba_org_data['cba_month'].str[:4] == str(last_year)) & (
            DF_cba_org_data['cba_month'] <= last_year_month_str)].groupby('cba_month').agg(
    ...
    d7 = d1_1[d1_1['cba_month'] == M].groupby('station_category').agg(
        {'gun_charging_volume_d': 'mean'}).reset_index().sort_values(by='gun_charging_volume_d', ascending=False).iloc[
        0]['station_category']
    data5.append(d7)
    data5
    ...
    d8 = \
        d1_1.groupby('city').agg({'gun_charging_volume_d': 'mean'}).reset_index().sort_values(
            by='gun_charging_volume_d',
    ...
    d9 = \
        pd.merge(d9, DF_province[['stat_time', 'avg_daily_energy']], left_on='month', right_on='stat_time', how='left')[
            ['month', 'gun_charging_volume_d', 'avg_daily_energy']]
    ...
    def parse_utilization_rate(value):
        """兼容数据库小数和带百分号的文本。"""
        if pd.isna(value):
            return np.nan
    ...
    DF_province['stat_time'] = (
        DF_province['stat_time'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
    )
    DF_province['avg_power_utilization'] = (
        DF_province['avg_power_utilization'].apply(parse_utilization_rate)
    )
    # 避免省级源表同月多行导致合并后的图表月份重复。
    DF_province = (
        DF_province.groupby('stat_time', as_index=False)['avg_power_utilization'].mean()
    )
    ...
    # 新表的容量利用率即功率利用率，保留旧列名以兼容后续输出逻辑。
    DF_cba_pue['cba_month'] = (
        DF_cba_pue['month'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
    )

    reported_pue = DF_cba_pue['capacity_utilization_rate'].apply(parse_utilization_rate)
    total_capacity = pd.to_numeric(DF_cba_pue['total_capacity_kwh'], errors='coerce')
    charging_energy = pd.to_numeric(DF_cba_pue['total_charging_energy_kwh'], errors='coerce')
    calculated_pue = charging_energy.div(total_capacity.where(total_capacity.ne(0)))

    # 源表利用率为空，或者为 0 但实际充电量大于 0 时，按总充电量/总容量回算。
    use_calculated = reported_pue.isna() | (
        reported_pue.eq(0) & calculated_pue.fillna(0).gt(0)
    )
    DF_cba_pue['pue'] = reported_pue.mask(use_calculated, calculated_pue)
    DF_cba_pue['pue'] = DF_cba_pue['pue'].replace([np.inf, -np.inf], np.nan)
    ...
    print('公司功率利用率按月校验：')
    print(
        DF_cba_pue.groupby('cba_month')['pue']
    ...
    def format_growth(current, base):
        if (
            pd.isna(current)
            or pd.isna(base)
            ...
            return '0.00'
        return f"{(float(current) / float(base) - 1) * 100:.2f}"
    ...
    current_pue = DF_cba_pue[DF_cba_pue['cba_month'] == M]
    pue_value_1 = current_pue['pue'].mean()
    pue_value = format_percent(pue_value_1)
    ...
    pue_last_year = format_growth(pue_value_1, pue_last_year_1)
    ...
    pue_prev = format_growth(pue_value_1, pue_prev_1)
    ...
    pue_this_year = format_percent(pue_this_year_1)
    ...
    pue_last_year_1 = \
        DF_cba_pue[(DF_cba_pue['cba_month'] <= last_year_month_str) & (DF_cba_pue['year'] == str(last_year))][
            'pue'].mean()
    pue_last_year = format_growth(pue_this_year_1, pue_last_year_1)
    result_list.append(pue_last_year)
    result_list
    ...
    province_current = DF_province.loc[
        DF_province['stat_time'] == M, 'avg_power_utilization'
    ]
    df_util = format_percent(province_current.iloc[0] if not province_current.empty else np.nan)
    ...
    station_category_rank = (
        current_pue.dropna(subset=['station_category'])
        .groupby('station_category')['pue'].mean().dropna().sort_values(ascending=False)
    )
    top_station_category = station_category_rank.index[0] if not station_category_rank.empty else ''
    ...
    station_city_rank = (
        current_pue.dropna(subset=['city'])
        .groupby('city')['pue'].mean().dropna().sort_values(ascending=False)
    )
    top_station_city = station_city_rank.index[0] if not station_city_rank.empty else ''
    ...
    df = pd.merge(d1, DF_province, left_on='month', right_on='stat_time', how='left')[
        ['month', 'pue', 'avg_power_utilization']]

    # In[615]:

    df['pue'] = pd.to_numeric(df['pue'], errors='coerce') * 100
    df['avg_power_utilization'] = (
        pd.to_numeric(df['avg_power_utilization'], errors='coerce') * 100
    )
    ...
    # 图表列表中使用 None 表示缺失值，避免入库 JSON 出现非标准的 NaN。
    for col in ['公司功率利用率', '全省站点功率利用率平均水平']:
        values = [None if pd.isna(value) else round(float(value), 2) for value in df[col]]
    ...
    DF1 = pd.DataFrame(
        columns=['month_data', 'yoy_growth_1', 'mom_growth', 'year_data', 'yoy_growth_2', 'prov_mean', 'top1_type',
                 'top1_city']
        , data=[result_list])
    ...
    d5_1 = round(float(DF_volume_1['off_peak_elec_cons'].sum()) / 10000, 2)
    if d5_1 != 0 :
    ...
    d6 = DF_volume.groupby('station_category').agg({'off_peak_elec_cons': 'sum'}).reset_index().sort_values(
        by='off_peak_elec_cons', ascending=False).iloc[0]['station_category']
    data7.append(d6)
    data7
    ...
    d7 = DF_volume.groupby('city').agg({'off_peak_elec_cons': 'sum'}).reset_index().sort_values(by='off_peak_elec_cons',
    ...
    # 综合计划完成情况的三个指标直接取数据库最新记录。
    sql = """
    select annual_target, year_to_date_completed, completion_rate
    from dp_target_progress
    order by id desc
    limit 1
    """
    DF_target_progress = SQL(sql)
    if DF_target_progress.empty:
        raise ValueError("dp_target_progress 缺少综合计划完成情况数据")
    ...
    """
    DF_staion = SQL(sql).fillna(0)
    DF_staion['charge_point_count'] = DF_staion['dc_charge_point_count'].fillna(0) + DF_staion[
        'ac_charge_point_count'].fillna(0)
    ...
    DF.loc[DF['title'] == '年度平台充电枪建设', 'completionRate'] = f"{d2:.2f}"

    # 下方年度充电量明细供后续四类站点统计复用。
    ...
    # 年度平台充电量：累加当年1月至分析月份的订单电量。
    DF_volume['trans_energy'] = pd.to_numeric(
        DF_volume['trans_energy'], errors='coerce'
    ).fillna(0)
    annual_charging_volume = round(
        float(DF_volume['trans_energy'].sum()) / 10000, 2
    )
    DF.loc[
        DF['title'] == '年度平台充电量', 'done'
    ] = annual_charging_volume

    annual_charging_target = pd.to_numeric(
        DF.loc[
            DF['title'] == '年度平台充电量', 'annualgoals'
        ],
        errors='coerce'
    ).iloc[0]
    if pd.notna(annual_charging_target) and float(annual_charging_target) != 0:
        annual_charging_rate = annual_charging_volume / float(annual_charging_target) * 100
    else:
        annual_charging_rate = 0
    DF.loc[
        DF['title'] == '年度平台充电量', 'completionRate'
    ] = f"{annual_charging_rate:.2f}"
    ...
    dict1 = [{'title': '综合计划完成情况',
              'content': [{'name': '年度目标', 'value': DF_target_progress.iloc[0]['annual_target'], 'unit': '万元'},
                          {'name': '本年累计已完成', 'value': DF_target_progress.iloc[0]['year_to_date_completed'],
                           'unit': '万元'},
                          {'name': '当前完成率', 'value': DF_target_progress.iloc[0]['completion_rate'], 'unit': '%'}]},
             ...
             {'title': '年度平台充电量', 'content': [{'name': '年度目标', 'value': DF.iloc[2, 1], 'unit': '万kWh'},
                                                     {'name': m1_str, 'value': DF.iloc[2, 2], 'unit': '万kWh'},
                                                     {'name': '当前完成率', 'value': DF.iloc[2, 3], 'unit': '%'}]}]
    dict1
    ...
    # 特殊说明：
    # 先通过charging_station.merchant_nature筛选电动公司的站点编号，
    ...
    # 因为站点归属和规则商户并不是完全一一对应，这里是特殊情况处理。
    ...
    data9['out'] = data9['investment_amount'] + data9['cost'] + (data9['parking_fee'] * data9['累计投运月份数']) + \
                   data9['merchant_profit_amount'] + data9['maintenance_cost']
    ...
    where cs.merchant_nature = '社会单位'
    and cs.operation_status in ('投运','停运','退运');
    """
    ...
    # 合并快慢枪数量
    DF_social_station['charge_point_count'] = DF_social_station['dc_charge_point_count'].fillna(0) + DF_social_station[
        'ac_charge_point_count'].fillna(0)
    # 处理投运时间
    DF_social_station['commissioning_year_month'] = DF_social_station['commissioning_time'].dt.strftime('%Y%m')
    # 累计接入充电枪统计投运和停运站点。
    d4 = DF_social_station[(DF_social_station['operation_status'].isin(['投运', '停运'])) & (
            DF_social_station['commissioning_year_month'] <= M)]['charge_point_count'].sum()
    d4
    ...
    DF_social_trans_energy = pd.merge(DF_social_station1, DF_volume[['station_no', 'trans_energy']], how='left',
                                      on='station_no')
    ...
    sql = """
    select * from charging_station as cs
    where cs.merchant_nature in ('产业单位','综合能源')
    and cs.operation_status in ('投运','停运','退运');
    """
    ...
    # 合并快慢枪数量
    df_industrial_unit_station['charge_point_count'] = df_industrial_unit_station['dc_charge_point_count'].fillna(0) + \
                                                       df_industrial_unit_station['ac_charge_point_count'].fillna(0)
    # 处理投运时间
    df_industrial_unit_station['commissioning_year_month'] = df_industrial_unit_station[
        'commissioning_time'].dt.strftime('%Y%m')
    # 累计接入充电枪统计投运和停运站点。
    d6 = df_industrial_unit_station[(df_industrial_unit_station['operation_status'].isin(['投运', '停运'])) & (
            df_industrial_unit_station['commissioning_year_month'] <= M)]['charge_point_count'].sum()
    d6
    ...
    df_industrial_unit_station1 = \
        df_industrial_unit_station[df_industrial_unit_station['commissioning_year_month'] <= M][['station_no']].copy()
    print(df_industrial_unit_station1.shape)
    DF_industrial_unit_trans_energy = pd.merge(df_industrial_unit_station1, DF_volume[['station_no', 'trans_energy']],
                                               how='left', on='station_no')
    ...
    # 合并快慢枪数量
    df_main_unit_station['charge_point_count'] = df_main_unit_station['dc_charge_point_count'].fillna(0) + \
                                                 df_main_unit_station['ac_charge_point_count'].fillna(0)
    # 处理投运时间
    df_main_unit_station['commissioning_year_month'] = df_main_unit_station['commissioning_time'].dt.strftime('%Y%m')
    # 累计接入充电枪统计投运和停运站点。
    d8 = df_main_unit_station[(df_main_unit_station['operation_status'].isin(['投运', '停运'])) & (
            df_main_unit_station['commissioning_year_month'] <= M)]['charge_point_count'].sum()
    d8
    ...
    df_main_unit_station1 = df_main_unit_station[df_main_unit_station['commissioning_year_month'] <= M][
        ['station_no']].copy()
    print(df_main_unit_station1.shape)
    DF_main_unit_trans_energy = pd.merge(df_main_unit_station1, DF_volume[['station_no', 'trans_energy']], how='left',
                                         on='station_no')
    ...
    # 累计接入充电枪统计投运和停运站点。
    d10 = DF_SCDD[(DF_SCDD['operation_status'].isin(['投运', '停运'])) & (DF_SCDD['commissioning_year_month'] <= M)][
        'charge_point_count'].sum()
    d10
    ...
    DF.loc[DF['title'] == '公司自建站点情况', ['chargingCable', 'chargingCapacity']] = [str(i) for i in
                                                                                        [int(d10), f"{d11:.2f}"]]
    DF.loc[DF['title'] == '社会桩接入情况', ['chargingCable', 'chargingCapacity']] = [str(i) for i in
                                                                                      [int(d4), f"{d5:.2f}"]]
    DF.loc[DF['title'] == '产业单位接入情况', ['chargingCable', 'chargingCapacity']] = [str(i) for i in
                                                                                        [int(d6), f"{d7:.2f}"]]
    DF.loc[DF['title'] == '省公司接入情况', ['chargingCable', 'chargingCapacity']] = [str(i) for i in
                                                                                      [int(d8), f"{d9:.2f}"]]
    ...
    d1_1 = DF_Business_Analysis[
        (DF_Business_Analysis['cba_month'].astype(str) <= str(M)) & (DF_Business_Analysis['year'] == str(year))]
    ...
    d2_1 = DF_Business_Analysis[(DF_Business_Analysis['cba_month'].astype(str) <= str(last_year_month_str)) & (
            DF_Business_Analysis['year'] == str(last_year))]
    ...
    d3_1_1 = DF_Business_Analysis[(DF_Business_Analysis['cba_month'].astype(str) <= str(M)) & (
            DF_Business_Analysis['year'] == str(year))][['rec_data_elec_fee_revenue']].sum() / 10000
    d3_1_1 = d3_1_1['rec_data_elec_fee_revenue']
    d3_1_2 = f'{d3_1_1 / d1_1:.2%}'
    d3_2_1 = DF_Business_Analysis[(DF_Business_Analysis['cba_month'].astype(str) <= str(M)) & (
            DF_Business_Analysis['year'] == str(year))][['rec_data_service_fee_revenue']].sum() / 10000
    ...
    # 计算总成本
    d1_1 = DF_Business_Analysis[
        (DF_Business_Analysis['cba_month'].astype(str) <= str(M)) & (DF_Business_Analysis['year'] == str(year))].copy()
    ...
    DF = pd.DataFrame(columns=['income', 'cost'],
                      data=[[data1.to_dict(orient='records')[0], data2.to_dict(orient='records')[0]]])
    DF['month'] = M
    ...
    data5_1 = DF_Business_Analysis.groupby('cba_month').agg(
        {'rec_data_elec_fee_revenue': 'sum', 'rec_data_service_fee_revenue': 'sum'}).reset_index()
    data5_1 = pd.merge(Data, data5_1, left_on='month', right_on='cba_month', how='left')[
        ['month', 'rec_data_elec_fee_revenue', 'rec_data_service_fee_revenue']]
    ...
    data5_1.rename(columns={'rec_data_elec_fee_revenue': '电费收入', 'rec_data_service_fee_revenue': '服务费收入'},
                   inplace=True)
    ...
    data6_1 = pd.merge(Data, data6_1, left_on='month', right_on='cba_month', how='left')[
        ['month', 'rec_cost_elec_fee', 'merchant_profit_amount', 'maintenance_cost']]
    ...
    data6_1.rename(
        columns={"rec_cost_elec_fee": '电费支出', 'merchant_profit_amount': '服务费分成', 'maintenance_cost': '运维费'},
        inplace=True)
    ...
    sequential_df['环比情况'] = (sequential_df['单枪充电收入'] - sequential_df['上月单枪充电收入']) / sequential_df[
        '上月单枪充电收入']
    sequential_df
